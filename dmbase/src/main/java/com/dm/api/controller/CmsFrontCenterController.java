...
public class CmsFrontCenterController {
	    ...
	    @Autowired SynVisitService synVisitService;
    ...
    String seed;
	 ...
	 @RequestMapping("/infoCenter.htm")
	    ...
	    public Map updateUser(UserAccount userAccount,String sex) {
		    ...
		    synVisitService.putInfo(user.getCode(), "12");
	    ...
	    public Map changePassword(UserAccount userAccount) {
			...
			user.setUpdateTime(new Date());
			try {
				user.setSynPassword(SimpleCryptoUtil.encrypt(seed, userAccount.getPassword()));
			} catch (Exception e) {
